...
function Verify() {
  ...
  const start = async (selectedCase=caseId) => {
    ...
    try { const r=await analyzeDocument(doc ?? undefined, selectedCase, selfie ?? undefined); setResult(r); addHistory(r); setRunning(false); nav("/result"); } catch { setRunning(false); setError("Screening could not be completed. Please try again."); }
  };
